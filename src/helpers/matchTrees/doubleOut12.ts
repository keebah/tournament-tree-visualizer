...
export const returnMatchTreeDoubleOut12 = (
  seeds: Team[],
  matchTree?: MatchTree
) => {
  const match1 = {
  ...
  const match11 = {
    team1: match7.winner,
    team2: match9.winner,
    winner: matchTree?.match11?.winner,
    loser: matchTree?.match11?.loser,
  };
  const match12 = {
    team1: match8.winner,
    team2: match10.winner,
    winner: matchTree?.match12?.winner,
    loser: matchTree?.match12?.loser,
  };
  const match13 = {
    team1: match11.winner,
    team2: match12.winner,
    winner: matchTree?.match13?.winner,
    loser: matchTree?.match13?.loser,
  };
  return {
    match1,
    match2,
